...
public class Controlador {
    ...
    public void controlador() {
        ...
        crearArbol(symbolo, soloNumero);
    }
    TreeNode root = null;
    private void crearArbol(LinkedList<String> symbolo, LinkedList<String> soloNumero) {
        ...
        String aux = symbolo.head.getInfo();
        for (int i = 0; i < symbolo.getSize(); i++) {
            symbol.add(new TreeNode<>(aux));
        }
        System.out.println("symbol = " + symbol.size());
        String aux1 = soloNumero.head.getInfo();
        for (int i = 0; i < soloNumero.getSize(); i++) {
            number.add(new TreeNode<>(aux));
        }
        ...
        while (symbol.size ()> 0) {// Cuando la cola de la matriz que almacena los símbolos es mayor que cero, el ciclo continúa. Nota: el número de símbolos es uno menos que el número
            ...
            number.add (0, s); // Agregue el nodo de símbolo s al primer lugar donde se almacena el nodo de número
        }
